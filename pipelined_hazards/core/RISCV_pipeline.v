...
module RISCV_pipeline (
    ...
    // When stalling, zero out the control bundle feeding ID/EX so the
    // in-flight instruction becomes a bubble.
    wire [7:0] id_ctrl_eff = stall ? 8'b0 : id_ctrl;
    ...
    register #(.N(155)) ID_EX (
        ...
        .rst (rst),
        .load(1'b1),
        .d   ({id_ctrl_eff, IF_ID_PC, rs1_data, rs2_data,
    ...
    forwarding_unit fu (
        ...
        .ex_mem_rd       (EX_MEM_Rd),
        .mem_wb_rd       (MEM_WB_Rd),
        .ex_mem_reg_write(EX_MEM_Ctrl[4]),
        .mem_wb_reg_write(MEM_WB_Ctrl[1]),
        .forward_a       (forward_a),
        .forward_b       (forward_b)
    );

    // 4:1 muxes selecting ALU operands (before ALUSrc).
    reg [31:0] forwarded_a, forwarded_b;
    always @(*) begin
        case (forward_a)
            2'b00:   forwarded_a = ID_EX_RegR1;
            2'b01:   forwarded_a = wb_data;
            2'b10:   forwarded_a = EX_MEM_ALU_out;
            default: forwarded_a = ID_EX_RegR1;
        endcase
        case (forward_b)
            2'b00:   forwarded_b = ID_EX_RegR2;
            2'b01:   forwarded_b = wb_data;
            2'b10:   forwarded_b = EX_MEM_ALU_out;
            default: forwarded_b = ID_EX_RegR2;
        endcase
    end

    wire [31:0] ex_alu_b = ex_alu_src ? ID_EX_Imm : forwarded_b;
    ...
    register #(.N(107)) EX_MEM (
        ...
        .d   ({ex_mem_ctrl_in, ex_branch_add_out, ex_zero,
               ex_alu_out, forwarded_b, ID_EX_Rd}),
        .q   (EX_MEM_q)
    );
